...
#if  DBG_I2C
#define I2C_DEBUG( X)   do {DPrint X ;} while(0)
#else
#define I2C_DEBUG( X)    
#endif
...
BOOL I2C1_I2CTxByte(INT8U Data)
{
	...
	while (cnt--)
	{
		...
		if (Data & 0x80)	GPIO_SetBits(IO_PORT_I2C1_SDA_DET, IO_PIN_I2C1_SDA_DET);
		else GPIO_ResetBits(IO_PORT_I2C1_SDA_DET, IO_PIN_I2C1_SDA_DET);
		...
		if((len != 0) && (pWriteData != NULL))
		{
			...
			if(ret == false){
				if(loop == 0){
					...
					#endif
					I2C_DEBUG((":> IIC∑¢ÀÕ ß∞‹: %s,%l\n",__FILE__,__LINE__));
					I2C_DEBUG((":> IIC∑¢ÀÕ ß∞‹: WriteData: [%h]\n",pWriteData,len));
					I2C_DEBUG((":> IIC∑¢ÀÕ ß∞‹: dest_add:0x%o,subaddr:%x\n",dest_add,subaddr));
				}
			}
			else break;
		}
...
BOOL I2C1_Rx(INT8U dest_add,INT32U subaddr,INT8U SizeOFsubaddr,INT8U *pReadData,INT16U len) 
{
	...
	if (!ret){
		I2C_DEBUG((":> IIC∂¡»°¥ÌŒÛ:%s,%l\n",__FILE__,__LINE__));
		I2C_DEBUG((":> IIC∂¡»°¥ÌŒÛ:dest_add:0x%o,subaddr:%x\n",dest_add,subaddr));
	}
	return ret;
}
...
BOOL I2C2_I2CTxByte(INT8U Data)
{
	...
	while (cnt--)
	{
		...
		if (Data & 0x80)	GPIO_SetBits(IO_PORT_I2C2_SDA_DET, IO_PIN_I2C2_SDA_DET);
		else GPIO_ResetBits(IO_PORT_I2C2_SDA_DET, IO_PIN_I2C2_SDA_DET);
